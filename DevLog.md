...
## 2025-03-15 因數統計法：合併 import、更新案例 C 註解與設計取捨說明

### 今天做了什麼

- 在 **replenishment_service.py** 採用「因數統計法」推算 Min/Multiple 後，整理程式與文件：合併重複的 import、更新測試案例註解，並在 DevLog 記錄「小樣本不另做嚴格模式」的設計取捨。

### 改了什麼

1. **`replenishment_service.py`**  
   - 合併 import：`math` 只保留一次，`from collections import Counter, defaultdict` 單行，移除多餘的 `import math` 與重複的 collections 匯入。  
   - 案例 B 註解：改為「因數統計法下 25 的因數得票少，12 達門檻為 Multiple」。  
   - 案例 C 註解：改為「預期 (2, 2)；2 為共同因數達門檻；小樣本不另做嚴格模式，避免過度擬合／過度設計」。  
   - 案例 D 註解維持原意（2 被排除、合法 Min=24）。
...
- 案例 C（[2, 3, 4]）樣本數極少，沒有足夠證據斷定 3 是雜訊；若為小樣本再加「絕對嚴格模式」（例如提高門檻或特殊分支），容易造成**過度擬合 (Overfitting)** 與**過度設計 (Over-engineering)**，目前因數統計法在一般情境下已足夠，故不新增小樣本嚴格模式，以註解與 DevLog 留下取捨理由即可。
